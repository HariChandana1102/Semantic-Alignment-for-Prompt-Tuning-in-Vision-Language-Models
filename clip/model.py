...
class Transformer(nn.Module):
    ...
    def forward(self, x,attr_bias=None):
        if attr_bias is None:
            return self.resblocks(x)
        else:
            return self.resblocks((x,attr_bias))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,attr_bias=None):
        ...
        if self.VPT_shallow:
            ...
            if attr_bias:
                visual_ctx = visual_ctx + attr_bias.unsqueeze(1)
                x = x.expand(attr_bias.shape[0],-1,-1)
        ...
        x = self.transformer(x,attr_bias)
        if type(x) is tuple:
            x = x[0]
...
def build_model(cfg,state_dict: dict, design_details):
